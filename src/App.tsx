...
function PreferitsCarret({ preferits, tallaRecomanada, onAfegir, onEliminarPreferit }: {
  ...
  return (
    <div style={{ marginTop: '50px' }}>
      <div style={{ borderTop: '1px solid #eceae4', paddingTop: '40px', marginBottom: '25px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ fontFamily: '"Didot", serif', fontSize: '22px', fontWeight: '300', margin: 0, letterSpacing: '2px' }}>
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: '"Didot", "Playfair Display", "Helvetica Neue", sans-serif', color: '#111', backgroundColor: '#faf9f6', minHeight: '100vh', margin: 0, padding: 0, position: 'relative' }}>
      ...
      {/* ALERTA DINÀMICA */}
      {missatgeWeb && (
        <div style={{ position: 'fixed', top: '90px', right: '30px', backgroundColor: missatgeWeb.tipus === 'exit' ? '#111' : '#bd1c1c', color: '#fff', padding: '15px 25px', zIndex: 200, fontSize: '13px', letterSpacing: '1px', boxShadow: '0 5px 20px rgba(0,0,0,0.15)', display: 'flex', alignItems: 'center', gap: '10px' }}>
      ...
      {/* CAPÇALERA */}
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '25px 50px', backgroundColor: '#ffffff', borderBottom: '1px solid #eceae4', position: 'sticky', top: 0, zIndex: 90 }}>
        <div style={{ fontSize: '28px', fontWeight: 'bold', letterSpacing: '6px', cursor: 'pointer', fontFamily: '"Didot", serif' }} onClick={() => { setProducteSeleccionat(null); setSeccioActiva('colleccio'); }}>
          MiRA
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '25px' }}>
          <div 
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '13px', letterSpacing: '1px', borderBottom: seccioActiva === 'carreto' ? '1px solid #111' : 'none', paddingBottom: '4px' }}
          >
            <ShoppingBag size={18} />
            <span>CARRETÓ ({carret.reduce((a, b) => a + b.quantitat, 0)})</span>
          </div>
        </div>
      </header>
      ...
      {seccioActiva === 'colleccio' && !producteSeleccionat && (
        <main style={{ padding: '60px 40px' }}>
          <div style={{ textAlign: 'center', marginBottom: '60px', maxWidth: '700px', margin: '0 auto 60px auto' }}>
            <h1 style={{ fontSize: '38px', fontWeight: '300', letterSpacing: '4px', marginBottom: '20px', fontFamily: '"Didot", serif' }}>EXPLORA LES LÍNIES</h1>
            ...
            {PRODUCTES.map((prod) => (
              <div key={prod.id} style={{ borderTop: '1px solid #eceae4', paddingTop: '40px' }}>
                <div style={{ marginBottom: '25px' }}>
                  ...
                  <h2 style={{ fontFamily: '"Didot", serif', fontSize: '28px', margin: '5px 0 0 0', fontWeight: '300', letterSpacing: '1px' }}>
                    {prod.id === 'pantalons-essence' ? 'LÍNIA ESSENCE' : 'LÍNIA TAILOR'}
                  </h2>
                </div>

                <div style={{ backgroundColor: '#ffffff', border: '1px solid #eae8e1', overflow: 'hidden', cursor: 'pointer', position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'center' }}>
                  <div onClick={() => { setProducteSeleccionat(prod); setImatgeActiva(0); }} style={{ width: '100%', height: '550px', backgroundColor: '#f5f5f3', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '50px', display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%', boxSizing: 'border-box' }} onClick={() => { setProducteSeleccionat(prod); setImatgeActiva(0); }}>
                    <h3 style={{ margin: '0 0 10px 0', fontSize: '22px', fontWeight: '400', letterSpacing: '1px' }}>{prod.nom}</h3>
      ...
      {seccioActiva === 'colleccio' && producteSeleccionat && (
        <main style={{ maxWidth: '1200px', margin: '40px auto', padding: '0 30px' }}>
          <button onClick={() => setProducteSeleccionat(null)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', fontSize: '13px', cursor: 'pointer', marginBottom: '40px', color: '#6d6b64', letterSpacing: '1px' }}>
            <ArrowLeft size={14} /> TORNAR A LA COL·LECCIÓ
          </button>

          <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '60px', alignItems: 'start' }}>
            <div>
              <div style={{ width: '100%', height: '620px', backgroundColor: '#f5f5f3', overflow: 'hidden', marginBottom: '20px', position: 'relative', border: '1px solid #eae8e1' }}>
                ...
                <button onClick={intentarObrirEmprovador} style={{ position: 'absolute', bottom: '25px', right: '25px', display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: '#111111', color: '#ffffff', border: 'none', padding: '14px 24px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px', letterSpacing: '2px' }}>
                  <Maximize2 size={15} /> PROVAR EN EMPROVADOR 3D
                </button>
              </div>

              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                {producteSeleccionat.imatges.map((img, index) => (
                  <div key={index} onClick={() => setImatgeActiva(index)} style={{ width: '75px', height: '95px', backgroundColor: '#f5f5f3', cursor: 'pointer', border: imatgeActiva === index ? '1px solid #111' : '1px solid transparent', padding: '2px' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <h1 style={{ fontSize: '36px', margin: '0 0 10px 0', fontWeight: '300', letterSpacing: '2px', fontFamily: '"Didot", serif' }}>{producteSeleccionat.nom}</h1>
      ...
      {seccioActiva === 'sobre-mira' && (
        <main style={{ maxWidth: '800px', margin: '60px auto', padding: '0 30px', textAlign: 'center' }}>
          <h1 style={{ fontSize: '36px', fontWeight: '300', letterSpacing: '4px', marginBottom: '30px', fontFamily: '"Didot", serif' }}>SOBRE MIRA</h1>
          ...
          <div style={{ backgroundColor: '#ffffff', padding: '40px', border: '1px solid #eae8e1', textAlign: 'left', marginBottom: '40px' }}>
      ...
      {seccioActiva === 'perfil' && (
        <main style={{ maxWidth: '1100px', margin: '40px auto', padding: '0 30px' }}>
          <h1 style={{ fontSize: '34px', fontWeight: '300', letterSpacing: '3px', marginBottom: '40px', fontFamily: '"Didot", serif' }}>EL MEU PERFIL DE LUXE</h1>
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '35px' }}>
                    <div>
                      ...
                      <input type="text" value={perfil.nom} onChange={e => setPerfil({...perfil, nom: e.target.value})} style={{ width: '100%', padding: '12px', border: '1px solid #ccc', backgroundColor: '#fafafa' }} />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold', marginBottom: '6px' }}>Correu Electrònic</label>
                      <input type="email" value={perfil.email} onChange={e => setPerfil({...perfil, email: e.target.value})} style={{ width: '100%', padding: '12px', border: '1px solid #ccc', backgroundColor: '#fafafa' }} />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold', marginBottom: '6px' }}>Número de Telèfon</label>
                      <input type="text" value={perfil.telefon} onChange={e => setPerfil({...perfil, telefon: e.target.value})} style={{ width: '100%', padding: '12px', border: '1px solid #ccc', backgroundColor: '#fafafa' }} />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold', marginBottom: '6px' }}>Direcció d'Enviament</label>
                      <input type="text" value={perfil.adreca} onChange={e => setPerfil({...perfil, adreca: e.target.value})} style={{ width: '100%', padding: '12px', border: '1px solid #ccc', backgroundColor: '#fafafa' }} />
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px', marginBottom: '25px' }}>
                    <div>
                      ...
                      <input type="number" value={perfil.alcada} onChange={e => setPerfil({...perfil, alcada: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc' }} />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', marginBottom: '6px' }}>Pes (kg)</label>
                      <input type="number" value={perfil.pes} onChange={e => setPerfil({...perfil, pes: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc' }} />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', marginBottom: '6px' }}>Pit (cm)</label>
                      <input type="number" value={perfil.pit} onChange={e => setPerfil({...perfil, pit: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc' }} />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', marginBottom: '6px' }}>Cintura (cm)</label>
                      <input type="number" value={perfil.cintura} onChange={e => setPerfil({...perfil, cintura: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc' }} />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', marginBottom: '6px' }}>Maluc (cm)</label>
                      <input type="number" value={perfil.maluc} onChange={e => setPerfil({...perfil, maluc: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc' }} />
                    </div>
                  </div>
                </div>
              )}
      ...
      {seccioActiva === 'carreto' && (
        <main style={{ maxWidth: '900px', margin: '40px auto', padding: '0 30px' }}>
          <h1 style={{ fontSize: '34px', fontWeight: '300', letterSpacing: '3px', marginBottom: '40px', fontFamily: '"Didot", serif', textAlign: 'center' }}>EL TEU CARRETÓ DE COMPRA</h1>
          ...
          {carret.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '40px', alignItems: 'start' }}>
              ...
              <div style={{ backgroundColor: '#fff', border: '1px solid #eae8e1', padding: '25px' }}>
                {carret.map((item, index) => (
                  <div key={`${item.producte.id}-${item.talla}`} style={{ display: 'flex', gap: '20px', padding: '20px 0', borderBottom: index === carret.length - 1 ? 'none' : '1px solid #eceae4', alignItems: 'center' }}>
                    <div style={{ width: '70px', height: '90px', backgroundColor: '#f5f5f3', overflow: 'hidden', border: '1px solid #eceae4' }}>
                      <img src={item.producte.imatges[0]} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <h4 style={{ margin: '0 0 5px 0', fontSize: '16px', fontWeight: '400' }}>{item.producte.nom}</h4>
                      <p style={{ margin: '0 0 5px 0', fontSize: '13px', color: '#6d6b64' }}>Talla seleccionada: <strong>{item.talla}</strong></p>
                      <p style={{ margin: 0, fontSize: '13px', color: '#111' }}>Quantitat: {item.quantitat} x {item.producte.preu.toFixed(2)} €</p>
                    </div>
              ...
              {/* Resum de compra */}
              <div style={{ backgroundColor: '#fff', border: '1px solid #eae8e1', padding: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '40px', alignItems: 'start' }}>
              <div style={{ backgroundColor: '#fff', border: '1px solid #eae8e1', padding: '35px' }}>
              ...
              <div style={{ backgroundColor: '#fff', border: '1px solid #eae8e1', padding: '30px' }}>
                ...
                {carret.map((item) => (
                  <div key={`${item.producte.id}-${item.talla}`} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '10px', color: '#444' }}>
                    <span>{item.producte.nom} ({item.talla}) x{item.quantitat}</span>
      ...
      {emprovadorObert && producteSeleccionat && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#ffffff', width: '90%', maxWidth: '1000px', height: '85vh', display: 'grid', gridTemplateColumns: '1fr 380px', position: 'relative' }}>
            <button onClick={() => setEmprovadorObert(false)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'none', border: 'none', cursor: 'pointer', color: '#111', zIndex: 510 }}>
              <X size={24} />
            </button>
            
            {/* Àrea del Model 3D Real */}
            <div style={{ backgroundColor: '#f5f5f3', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {React.createElement('model-viewer' as any, {
            ...
            <div style={{ padding: '40px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', borderLeft: '1px solid #eceae4', backgroundColor: '#fff', overflowY: 'auto' }}>
              <div>
                <span style={{ fontSize: '11px', letterSpacing: '2px', color: '#6d6b64', fontWeight: 'bold' }}>SISTEMA INTERACTIU MIRA</span>
                <h3 style={{ fontFamily: '"Didot", serif', fontSize: '24px', margin: '5px 0 15px 0', fontWeight: '300' }}>EMPROVADOR 3D</h3>
                <p style={{ fontSize: '13px', color: '#555', lineHeight: '1.6', marginBottom: '20px' }}>Estàs visualitzant l'adaptació digital en tres dimensions de la peça. Pots rotar, fer zoom i analitzar la caiguda estructural del teixit de forma interactiva.</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {/* BOTÓ AFEGIR AL CARRET DES DE L'EMPROVADOR */}
                <button
      ...
      {pasFotoObligatori && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#fff', padding: '40px', maxWidth: '480px', width: '90%', border: '1px solid #eae8e1', textAlign: 'center' }}>
      ...
      {mesuresRapidesObertes && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 650, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#fff', padding: '40px', maxWidth: '460px', width: '90%', border: '1px solid #eae8e1', position: 'relative' }}>
      ...
      {guiaMidesOberta && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#fff', padding: '40px', maxWidth: '480px', width: '90%', position: 'relative', border: '1px solid #eae8e1' }}>
